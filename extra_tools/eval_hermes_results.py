    ...
    def compute_scores(self, pred_path, key, future=False):
        ...
        for file in tqdm(file_list, desc="Loading results"):
            with open(os.path.join(pred_path, file), 'r') as f:
                ...
                # Chamfer: accumulate per available frame (partial scenarios included)
                if not data.get('ignore_flag', False):
                    any_present = False
                    mapping = [
                        ("chamfer_dis_frame_0", "0"),
                        ("chamfer_dis_frame_2", "1"),
                        ("chamfer_dis_frame_4", "2"),
                        ("chamfer_dis_frame_6", "3"),
                    ]
                    for key_name, t_str in mapping:
                        if key_name in data:
                            chamfer[t_str].append(data[key_name])
                            any_present = True
                    if any_present:
                        valid_count += 1
