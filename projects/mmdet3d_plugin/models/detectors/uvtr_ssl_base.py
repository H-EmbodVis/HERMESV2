...
class UVTRSSLBase(MVXTwoStageDetector):
    ...
    def extract_img_feat(self, img, img_metas, len_queue=None):
        ...
        if self.with_img_neck:
            img_feats = self.img_neck(img_feats)
            # img_feats = checkpoint(self.img_neck, img_feats, use_reentrant=False)

        img_feats_reshaped = []
        for img_feat in img_feats:
            img_feat = self.input_proj(img_feat)
            # img_feat = checkpoint(self.input_proj, img_feat, use_reentrant=False)
            BN, C, H, W = img_feat.size()
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        if hasattr(self, "pts_voxel_encoder") and self.training:
    ...
    def simple_test(self, img_metas, points=None, img=None):
        ...
        if count >= 6000:
            print("Reached 6000 files, exiting...")
            print("-" * 10)
            print(f"for first {count} files, ")
            print("avg chamfer_dis: ", sum(chamfer_dis_list) / len(chamfer_dis_list))
            print("avg_depth_l1: ", sum(depth_l1_list) / len(depth_l1_list))
