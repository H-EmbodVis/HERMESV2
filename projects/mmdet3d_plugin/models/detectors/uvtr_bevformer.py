...
class UVTRBEVFormer(UVTRSSLBase):
    ...
    def forward_pts_train(
        ...
        out_dict = self.pts_bbox_head(
            ...
            fusion_prev=True if any(f < 0 for f in self.frames) else False
        )
    ...
    def obtain_history_bev(self, imgs_queue, img_metas_list):
        ...
        with torch.no_grad():
            ...
            for t in range(len_queue):
                ...
                bev = self.pts_bbox_head.get_uni_feats(img_feats=img_feats, img_metas=img_metas, prev_bev=None,
                                                       only_bev=True, bs=1, dtype=img_feats[0].dtype)
                prev_bev[self.frames[t]] = bev.detach()
