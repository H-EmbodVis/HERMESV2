...
class HERMES(UVTRSSLBase):
    ...
    def simple_test(self, img_metas, points=None, img=None):
        ...
        # Optional fast path: skip already processed samples by existing json.
        skip_existing = os.environ.get("SKIP_EXISTING_RESULTS", "0").lower() in ("1", "true", "yes")
        if skip_existing:
            sample_idx = str(img_metas[self.frames[0]].get('sample_idx', ''))
            ...
            result_path = os.path.join(save_dir, "results", f"{sample_idx}.json")
            if sample_idx and os.path.exists(result_path):
                print(f"[skip] sample already processed: {sample_idx}")
                return [None, ]
