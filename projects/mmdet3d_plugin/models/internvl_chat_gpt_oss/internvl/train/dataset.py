...
def preprocess_qwen3(
    ...
    '''
    Preprocess the input data for Qwen3 model, adopted from InternVL2.5 preprocessing.
    '''
    assert len(sources) == 1, 'process only the first conversations'
    conversations = sources[0]
...
def preprocess_llama3(
    ...
    '''
    Preprocess the input data for Llama 3 model.
    '''
    ...
    if system_prompt is not None:
        sys_str = f"{begin_of_text}<|start_header_id|>system<|end_header_id|>\n\n{system_prompt}<|eot_id|>"
        batches.append(sys_str)
        roles.append('system')
    ...
    for i, conversation in enumerate(conversations):
        ...
        prefix = ""
        if i == 0 and system_prompt is None:
            prefix = begin_of_text

        if role == 'human':
            # User Block
            msg = f"{prefix}<|start_header_id|>user<|end_header_id|>\n\n{content}<|eot_id|>"
            batches.append(msg)
            roles.append('human')
        elif role == 'gpt':
            # Assistant Block
            msg = f"{prefix}<|start_header_id|>assistant<|end_header_id|>\n\n{content}<|eot_id|>"
            batches.append(msg)
            roles.append('gpt')
        else:
            raise NotImplementedError
    ...
    assist_header_str = "<|start_header_id|>assistant<|end_header_id|>\n\n"
    assist_header_ids = tokenizer(assist_header_str, add_special_tokens=False, return_tensors='np').input_ids[0]
    assist_header_len = assist_header_ids.shape[0]
    ...
    for role, input_id in zip(roles, input_ids):
        ...
        if role == 'system' or role == 'human':
            ...
            target = input_id.copy()
            if len(target) > assist_header_len:
                target[:assist_header_len] = IGNORE_TOKEN_ID
            else:
...
def preprocess_internvl3_5_gpt_oss(
    ...
    for conversation in conversations:
        if conversation['from'] == 'human':
            ...
            eos = '<|return|>' if i == len(
                conversations) - 1 else '<|end|>'  # '<|return|>' if last message else '<|end|>'
    ...
    for conversation in conversations:
        if conversation['from'] == 'human':
            ...
            if '<think>' in value and '</think>' in value:
                ...
                batches.append(
                    f'<|start|>assistant<|channel|>analysis<|message|>{think}<|end|><|start|>assistant<|channel|>final<|message|>{final}<|return|>')
            else:
