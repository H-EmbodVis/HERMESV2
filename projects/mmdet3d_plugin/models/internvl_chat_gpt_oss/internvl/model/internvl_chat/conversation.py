...
class Conversation:
    ...
    def get_prompt(self) -> str:
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            wrap_answer = lambda msg: f"<|start_header_id|>assistant<|end_header_id|>\n\n{msg}"
            ret = ""
            ...
            for i, (role, message) in enumerate(self.messages):
                ...
                if message:
                    ...
                    if i == 0 and system_prompt:
                        ret += self.sep + wrap_sys(system_prompt)+ self.sep2
                    if i % 2 == 0:
                        # message = wrap_inst(message)
                        ret += role + message + self.sep2 + self.roles[1]  # add assistant role
                    else:
...
register_conv_template(
    Conversation(
        name='hermes_llm',
        system_template='<|im_start|>system\n{system_message}',
        system_message="You are HERMES, an expert driving assistant. Your role is to understand the current situation, predict how the scene will evolve, and identify potential hazards. /no_think",
        roles=('<|im_start|>user\n', '<|im_start|>assistant\n'),
        sep_style=SeparatorStyle.MPT,
        sep='<|im_end|>\n',
    )
