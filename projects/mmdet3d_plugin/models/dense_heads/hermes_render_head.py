...
class HERMESRenderHead(BEVFormerRenderHead):
    ...
    def forward(self, pts_feats, img_feats, rays, img_metas, img_depth, prev_bev=None, only_bev=False,
        ...
        """
        bs = len(img_metas)
        if img_feats is not None and pts_feats is None:
            dtype = img_feats[0].dtype
            if self.use_downsample:
                uni_feats, bev_embed_ori, bev_embed_recon = self.get_uni_feats(img_feats, img_metas, prev_bev, only_bev,
                                                                               bs, dtype, fusion_prev=fusion_prev)
            else:
                uni_feats = self.get_uni_feats(img_feats, img_metas, prev_bev, only_bev, bs, dtype,
                                               fusion_prev=fusion_prev)
                if only_bev:
                    return uni_feats
            uni_feats = self.render_conv(uni_feats)

        elif pts_feats is not None and img_feats is None:
            uni_feats = self.render_conv(pts_feats)

        elif pts_feats is not None and img_feats is not None:
            # align the image features to frozen pts features
            dtype = img_feats[0].dtype
            if self.use_downsample:
                uni_feats, bev_embed_ori, bev_embed_recon = self.get_uni_feats(img_feats, img_metas,
                                                                               prev_bev, only_bev, bs, dtype,
                                                                               fusion_prev=fusion_prev)
            else:
                uni_feats = self.get_uni_feats(img_feats, img_metas, prev_bev, only_bev, bs,
                                               dtype, fusion_prev=fusion_prev)
                if only_bev:
                    return uni_feats
            # compute the cosine similarity loss to align the features
            if self.training:
                if uni_feats.shape[-1] != pts_feats.shape[-1]:
                    self.sim_loss = 1 - F.cosine_similarity(
                        F.interpolate(uni_feats, size=pts_feats.shape[2:], mode='trilinear',
                                      align_corners=False).float(), pts_feats.float(), dim=1).mean()
                else:
                    self.sim_loss = 1 - F.cosine_similarity(uni_feats.float(), pts_feats.float(), dim=1).mean()
            else:
                self.sim_loss = None
            uni_feats = self.render_conv(uni_feats)
