...
class BEVFormerRenderHead(DETRHead):
    ...
    def get_uni_feats(self, img_feats, img_metas, prev_bev, only_bev, bs, dtype, fusion_prev=False):
        ...
        if fusion_prev and prev_bev is not None:
            if len(self.frames) > 1:
                ...
                bev_embed = [x.reshape(x.shape[0], self.bev_h, self.bev_w, x.shape[-1]).permute(0, 3, 1, 2).contiguous() for x in
                             prev_bev]
                bev_embed = prev_bev[cur_ind] + self.transformer.fusion(bev_embed)

                # bev_embed = self.transformer.fusion(bev_embed)
                # if not self.training:
                #     # save as npy
                #     np.save(f"bev_embed_fusion.npy",
                #             bev_embed.permute(0, 2, 1).contiguous().view(-1, self.bev_h, self.bev_w).cpu().numpy())
        ...
        uni_feats = self.bev_upsample(bev_embed)
