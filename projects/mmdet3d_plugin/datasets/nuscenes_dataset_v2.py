...
class CustomNuScenesDatasetV2(NuScenesDataset):
    ...
    def union2one(self, queue: dict):
        ...
        prev_pos = None
        prev_angle = None
        start_pos = copy.deepcopy(queue[0]['img_metas'].data['can_bus'][:3])
        start_angle = copy.deepcopy(queue[0]['img_metas'].data['can_bus'][-1])
        start_scene_token = queue[0]['img_metas'].data['scene_token']
